...
2,450 passed tests and 17 ignored; the current MySQL frontend has 167 passing
...
`turso_mysql_server` unit suite has 335 tests, the checkpoint-authority library
...
per batch. Preparing and rejected target-trigger forms do not reserve, while
rollback burns IDs. Qualified names, `TEMPORARY`, wider writes, `ALTER`,
`VACUUM` lifecycle, and protocol INSERT/OK paths remain gated. A successful
generated insert records its first ID in dedicated connection-local MySQL state;
failure and rollback preserve it, and checked `SELECT LAST_INSERT_ID()` reads it
at execution time. D007's registry

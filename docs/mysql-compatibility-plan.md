...
10. D025/D026 provide the standalone `turso-mysql-offline-provision` Unix CLI
   ...
   restarts the authority, and reopens revision one. Real-service retained-
   journal reconciliation and real-service recovery at every crash boundary
   remain gated. The privileged Linux fixture covers normal cross-UID
...
Certificate/trust policy, real-service retained-journal reconciliation,
real-service recovery at every crash boundary, the
...
`add-account`. Real-service retained-journal reconciliation and recovery at
each crash boundary remain validation work in parallel.

...
5. A default-deny authorization wrapper that checks the principal before any
   database lookup, selection, query, create, drop, or list operation.
...
`SELECT` subset. The Unix adapter
now owns a registry-backed session, routes `COM_INIT_DB` through it, and uses
the same selector before the final authentication OK when the handshake names
an initial database. A production socket/TLS runtime and authorization policy
remain absent.
...
2,450 passed tests and 17 ignored; the current MySQL frontend has 144 passing
...
stack has 125. Core has 11 focused allocator tests, 16 with
...
complete-frame owner retains this state internally and drops it on close or
error. Production credential storage, authorization, and socket/TLS transport
are not implemented.
...
result memory. Its registry-backed Unix form implements `COM_INIT_DB`, preserves
the old selection after a failed switch, returns typed 1046/1049 categories,
and gates both fast and full authentication OK packets on successful handshake
database selection. The server requires every nonzero client response-packet limit
...
root lock remain retained by Core's lifetime guard. `COM_INIT_DB` and handshake
database selection now use this session through the transport-neutral adapter,
including typed 1046/1049 errors and fast/full-auth selection gates. The strict
...
catalog operations. Network `COM_QUERY` keeps them closed until authorization
is wired. There is still no production server runtime owner. The preopened
...
Next smallest vertical slice: add a one-shot post-authentication executor
factory and a default-deny authorization wrapper before any catalog access.
Only after
that wrapper passes existence-leak and privilege-revocation tests may the
checked database admin commands be connected to network `COM_QUERY`. A

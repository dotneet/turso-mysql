...
9. D024 adds the separate local checkpoint authority. Its foreground
   ...
   are present. Privileged cross-UID CI, deterministic crash-point injection,
   and TCP/TLS remain gated.
10. D025 adds the standalone `turso-mysql-offline-provision` Unix CLI for one
   ...
   deadline. TTY prompt cancellation restores echo and prior `SIGINT`,
   `SIGTERM`, and `SIGHUP` handlers; stdin/FD accept FIFO/socket input only,
   never regular files, terminals, or device files. The journal is durable
   before snapshot publish; recovery clears a snapshotless journal only after
   authority GET reports `Missing`. Its coordination deadline bounds
   provisioning-lock waits and that GET, not filesystem publication as a total
   wall-clock promise, and secret input does not consume it. Database grants,
   replacement, and privileged real cross-UID testing remain gated.
...
privileged cross-UID/crash testing, journal-backed replacement/database-grant
...
reconciliation for ambiguous CAS results. The next P5 priority is privileged
real cross-UID and deterministic crash-point gates for the authority and D025
workflow; they validate the isolation and crash claims already exposed by the
first-account slice. Only after those gates should the next functional slice
add database grants and replacement, and it must use the same durable pending
journal before making any process-crash recovery claim. The public Unix runtime

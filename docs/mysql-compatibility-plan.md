...
1. Strict signed `TINYINT`, `SMALLINT`, `MEDIUMINT`, `INT`, and `BIGINT` assignment over
   existing i64 storage, including statement rollback and typed range errors.
2. Further signed widths, then unsigned widths only after an exact `u64`
...
- Implement the first `SHOW COLUMNS`/`DESCRIBE` catalog slice: plain syntax only,
  selected database required, database-level `Query` authorization, metadata
  from verified normalized MySQL DDL, typed defaults, and bounded six-column
  result conversion.
...
2. Implement the five-artifact registry with `lower_case_table_names=1`,
   opaque file identities, v2 metadata sidecars, a durable allocator sidecar, durable
...
6. Add plain `SHOW TABLES` after database selection and authorization. Read the
   ...
   `IN`, `LIKE`, and `WHERE`. When database-wide `Query` is denied, filter the
   result through table `Select` grants. Bound the scan and retained result
   before protocol encoding.
7. Add plain `SHOW COLUMNS`/`DESCRIBE` after the same authorization check. Read
   only verified normalized MySQL DDL and checked Core schema state, preserve
   typed defaults, return exactly the six bounded metadata columns, and reject
   unsupported or corrupt definitions without guessing.
...
9. D024 adds the separate local checkpoint authority. Its foreground
   ...
   are present. The pinned-Docker Linux CI gate builds the standalone
...
10. D025/D026 provide the standalone `turso-mysql-offline-provision` Unix CLI
   ...
   `create`, and `drop` permissions. `--table-grant DATABASE.TABLE:select`
   accepts canonical table names only, requires global and matching database
   `connect`, and is validated before password input. `add-account`
   ...
   caller. D031 kills initialization and account-addition children at all four
   durable boundaries against the real same-UID authority, restarts before
   reconciliation, and restarts again before exact open. Its stop hooks use a
...
connections, reconnect, pool checkout/reset (including rollback, prepared
statement invalidation, and `LAST_INSERT_ID()` clearing), clean disconnect,
and runtime endpoint cleanup after `SIGTERM`. It is an ignored test that
...
| D004 | P3 | Which exact numeric representation handles MySQL `DECIMAL`? | first implement strict signed integer assignment checks over i64 storage; reconstruct a typed `MySqlNumericSpec` from durable DDL and keep unsigned/DECIMAL fail-closed. DECIMAL may reuse the blob codec only with a separate MySQL half-up round-then-overflow implementation and exact comparator; never use the generic f64 overflow fallback, truncating `with_scale`, or `Value::as_uint()` reinterpretation | strict signed `TINYINT`/`SMALLINT`/`MEDIUMINT`/`INT`/`INTEGER`/`BIGINT` assignment is implemented for marked-table INSERT/UPDATE. The pre-storage validator is database-aware for main/TEMP/attached schemas and covers parameters, multi-row rollback, triggers, reopen, and VACUUM; the pinned MEDIUMINT case/golden covers both signed boundaries, NULL, and insert/update overflow, while BIGINT retains the signed i64 extrema. Full coercion, further signed widths, unsigned types, DECIMAL, permissive saturation/warnings, casts/arithmetic/order, broader numeric metadata, protocol errors, and transaction diagnostics remain separate gated slices |
...
| D029 | P5 | Does the normal granted account-addition path preserve the checkpoint authority's separate-UID boundary? | extend the privileged pinned-Linux fixture through CLI `add-account`, revision-one reopen, and existing foreign-peer rejection | implemented in the privileged Docker gate. The client UID initializes and appends granted accounts, then verifies the authority and account snapshot at revision one; the foreign UID remains rejected despite shared group access and `SIGTERM` still removes the endpoint. The local macOS run validates shell syntax and compiles the ignored test, but cannot execute Linux ELF artifacts. Distinct-UID interrupted recovery remains gated. |
| D030 | P5 | Can a retained replacement journal converge when the real authority made the CAS durable but its reply was ambiguous? | wrap the real Unix client in tests so it delegates the durable CAS but reports `Ambiguous`, then reconcile with a fresh normal client | implemented. The failed add-account call retains its exact journal while the real authority is already at revision one. A fresh client repeats the idempotent CAS, clears the journal, and reopens both accounts after authority restart. D031 covers each real-service crash boundary. |
| D031 | P5 | Does the journal workflow recover from a killed client at every durable boundary against the real authority process? | run initialization and account addition in child processes with default-off test hooks; observe `SIGSTOP`, send `SIGKILL`, restart the authority, reconcile the exact state, restart again, and open only the resulting exact generation | implemented for all eight same-effective-UID cases: journal publication, snapshot publication, durable CAS, and journal removal for both operations. The hooks are unavailable in default production builds and must remain disabled there. The distinct-UID Linux crash matrix remains gated. |
...
standard input. `verify-p0` requires Docker/Compose and the
`MYSQL_CONFORMANCE_PASSWORD`, `MYSQL_CONFORMANCE_ROOT_PASSWORD`, and
`MYSQL_ORACLE_DSN` environment variables; test output must redact credentials.
...
Recent committed slices add strict signed `SMALLINT`, `MEDIUMINT`, and `BIGINT`
assignment checks, typed persisted defaults, authorized bounded
`SHOW COLUMNS`/`DESCRIBE` results with primary/auto-increment display metadata,
the committed `show-columns` case and pinned golden, `COM_RESET_CONNECTION` plus stale
...
constructor is now used by the Unix and crate-private TCP owners. A concrete frontend adapter executes
...
excluding internal tables. The narrow checked `information_schema.TABLES`
projection/filter/order query follows the same selected-database authorization
and returns bounded user table/view metadata; its MySQL oracle case is a
reference contract listed in the P0 manifest, not a Turso execution gate.
`SHOW COLUMNS`/`DESCRIBE`/`DESC` now return the bounded six-column metadata
result from verified DDL, including typed defaults and `PRI`/`auto_increment`
for the checked primary auto-increment form. Frontend metadata preserves the
declared `INT` versus `INTEGER` spelling, while the wire `Type` column
canonicalizes both to `int`; the pinned case/golden is committed.
Canonical marked views also support only bare direct projections from one base
table. Persisted view rootpage, SQL, and base-column provenance are checked;
...
The checked DML parser now accepts conservative unqualified `DELETE` and
one-table `UPDATE` forms and rejects aliases, joins, multi-table sources,
modifiers, ordering, limits, returning, and other unproven forms. The checked
COM_QUERY path executes ordinary INSERT/DELETE/UPDATE and returns affected
rows plus the first generated ID for the narrow AUTO_INCREMENT INSERT. UPDATE
uses changed rows by default and matched rows after `CLIENT_FOUND_ROWS`
negotiation. Core leaves SQLite `changes()` as the matched-row count and uses a
separate success-only MySQL changed-row counter that compares the old and new
stored key and record in both WAL and MVCC execution paths. Checked writes
carry one deadline across their stages,
...
binary-safe text-row, and negotiated EOF/OK terminator codecs.
Packet-write batch staging is bounded by queued frame and byte limits, and a
rejected batch leaves the queue unchanged.
Signed Int8, Int16, Int32, and Int64 binary result primitives use checked
widths and NULL-bitmap-safe decoding; signed `MYSQL_TYPE_LONGLONG` extrema
coverage is also committed. The dispatcher
...
4,096-row and retained-result bounds; when database-wide `Query` is denied,
table `Select` grants filter the result. `FULL`, `FROM`, `IN`, `LIKE`, and
`WHERE` remain unsupported.
The narrow checked `information_schema.TABLES` query accepts only the exact
projection/filter/order shape, performs the same selected-database `Query`
authorization, and returns bounded user table/view metadata; when database-wide
`Query` is denied, table `Select` grants filter the result. Its checked MySQL
oracle case is a reference contract listed in the P0 manifest, not a Turso
execution gate. Other `information_schema` providers and filtering beyond this
selected-database path remain incomplete.
The committed metadata slice adds plain `SHOW COLUMNS FROM table`,
`DESCRIBE table`, and `DESC table` through selected-database `Query` authorization
or an exact table `Select` grant when database-wide `Query` is denied, with six
bounded metadata columns and typed persisted defaults. Catalog rows are filtered
through table grants on the narrow selected-database paths.
It also supports only canonical marked views that directly project columns from
one base table, verifying persisted view/rootpage/column provenance, preserving
projected type and nullable metadata, and clearing view `Key`, `Default`, and
`Extra`; chains, aliases, expressions, joins, qualified or system sources, and
duplicate projections are rejected.
The checked primary auto-increment form renders `PRI` and `auto_increment`,
preserving the declared `INT` versus `INTEGER` spelling in frontend metadata;
the wire `Type` column canonicalizes both to `int`. Unknown extras fail closed.
The pinned case/golden is committed. Malformed, unsupported, or corrupt
...
form. Registry-selected embedded sessions execute the narrow unqualified,
explicit-column, direct-literal `INSERT ... VALUES` form with one durable range
per batch. Prepared protocol execution also accepts bare `?` values in that
omitted-ID shape, preserving their order and reserving, injecting, repreparing,
and binding only at execute time. Preparing and rejected target-trigger forms do
not reserve, while rollback and failed execution burn IDs. Qualified names,
`TEMPORARY`, named or numbered markers, expression values, explicit allocator
columns, wider writes, `ALTER`, and `VACUUM` lifecycle remain gated. A
successful generated insert records its first ID in dedicated connection-local
MySQL state; failure and rollback preserve it, and checked `SELECT
LAST_INSERT_ID()` reads it at execution time. D007's registry
now owns five artifacts per database: the raw main file, raw WAL, separate
main-info and wal-info sidecars, and a durable AUTO_INCREMENT allocator. The strict v2 metadata records are fixed at 61
...
remains outside the trusted-root threat model. Shared-WAL/MVCC authority still
requires a separate later capability; narrow protocol DML exposure is now
implemented. There is no rollback-journal writer to model in this slice.
...
Historical validation note (not a current test claim): an earlier snapshot
recorded a single-thread whole-core run with 2,450 tests and 17 ignored, while
the default parallel core run had a known flaky MVCC abort. This documentation
update did not rerun those suites. Earlier integration regressions were closed
by the existing process-wide file identity and remove/recreate coverage. The
local machine does not have `cargo-deny`, so the dependency license result
relies on the recorded audit rather than a local `cargo deny check licenses`
run.
...
replacement journal. Same-UID real-service process-kill tests now cover all
four initialization and all four account-addition boundaries. The equivalent
distinct-UID Linux crash matrix remains validation work.
...
For D006, the next slices are wider checked write forms, explicit exhaustion
diagnostics, and `VACUUM` lifecycle. D005 can proceed only
after the frozen UCA9/CLDR30 data-generation and notices pipeline is reproducible.
Production TCP/TLS serving comes after the certificate/trust policy is
concrete; the protocol-wired Unix library boundary does not depend on that
TCP-only policy. Do not

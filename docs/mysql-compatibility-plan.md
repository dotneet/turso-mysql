...
11. The standalone Unix-only `turso-mysql-server` executable validates all
    ...
    authentication, `USE`, ordinary DML, prepared DML, reads, and `SIGTERM`
    endpoint cleanup. The fixture is wired into CI but was not run locally for
    the current validation.
12. Prepared statement prepare, metadata, execute, reset, close, binary
   parameters, and binary result rows.
13. Optional multi-statements and cancellation/resource controls.
...
additions with database grants. The standalone Unix-only
`turso-mysql-server` runtime is implemented. TCP/TLS transport and certificate
policy remain absent or undecided; account/grant edits or
...
database grants. The standalone Unix-only `turso-mysql-server` runtime is
implemented. TCP/TLS transport and certificate policy are not implemented or
decided; account/grant edits or
removals remain future provisioning work.
...
TCP/TLS still block a deployable server. The

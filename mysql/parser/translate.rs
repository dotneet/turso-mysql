...
) -> Result<(&'static str, Option<&Expr>), ParseError> {
    use sqlparser::ast::{JoinConstraint, JoinOperator};
    match operator {
        JoinOperator::CrossJoin(JoinConstraint::None) => Ok(("CROSS JOIN", None)),
        JoinOperator::Join(JoinConstraint::On(expr))
        | JoinOperator::Inner(JoinConstraint::On(expr)) => Ok(("JOIN", Some(expr))),
        JoinOperator::Left(JoinConstraint::On(expr))
        | JoinOperator::LeftOuter(JoinConstraint::On(expr)) => Ok(("LEFT JOIN", Some(expr))),
        JoinOperator::Right(JoinConstraint::On(expr))
        | JoinOperator::RightOuter(JoinConstraint::On(expr)) => Ok(("RIGHT JOIN", Some(expr))),

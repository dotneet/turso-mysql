...
post-authentication wrapper are present. Offline provisioning and pure runtime
security configuration exist as library boundaries. Trusted embedded sessions and
...
| `caching_sha2_password` | n/a | n/a | experimental | experimental | partial | [`verifier`](server/src/verifier.rs), [`persistent account store`](server/src/persistent_account_store.rs), [`offline provisioning`](server/src/offline_provisioning.rs), [`authentication state`](server/src/auth.rs) | Constant-time verification uses one owned, zeroizing provider snapshot across fast/full auth and mints an opaque canonical account principal only on success. The persistent Unix store keeps only full verifiers and the matching privileges in one bounded, CAS-published generation; deleted account IDs remain retired, invalid reloads keep the last good generation, and open plus reload require exact equality with an external store-ID/revision/digest checkpoint. Offline provisioning clears borrowed password buffers and acknowledges only after external checkpoint CAS. V1 is exact username-only. No provisioning executable, at-rest encryption, external checkpoint service, certificate policy, runtime wiring, or network transport exists. |
...
| TCP/TLS and Unix-socket listeners | n/a | n/a | planned | planned | planned | [`runtime config`](server/src/runtime_config.rs), [protocol architecture](../docs/mysql-compatibility-mode.md) | Pure configuration requires TCP TLS references, same-effective-UID Unix peer policy, an external checkpoint authority, and bounded limits/timeouts. Stream framing and a complete-frame connection owner exist, but no listener, TLS engine, certificate policy, checkpoint reader, peer inspection, reload scheduler, or serial worker runtime exists. |

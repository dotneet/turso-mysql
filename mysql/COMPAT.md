...
Unix-only `turso-mysql-offline-provision` binary now initializes one account or
reconciles its first-initialization journal. It requires explicit root,
authority, UID, and timeout configuration; accepts exactly one protected
password source plus an initialize-only absolute password-input timeout; and
has fixed redacted output with exits `0`/`2`/`3`/`4`/`5`. Password collection
does not consume the separate coordination deadline. TTY restores echo and
prior `SIGINT`/`SIGTERM`/`SIGHUP` handlers after `tcflush(TCIFLUSH)`; stdin/FD
accept FIFO/socket input only, temporarily use `O_NONBLOCK` through the
absolute input deadline, and restore original flags before returning. A
deterministic test stops the initialization child with `SIGSTOP`, sends
`SIGKILL`, and reconciles it at journal publication, snapshot publication,
durable CAS, and journal removal. A separate test-only one-shot matrix covers
all sixteen before/after write, file-sync, rename, and directory-sync faults
for journal and snapshot publication; it checks exact final states,
reconciliation, no temporary remnants, and exact checkpoint reopen.
It has no database-grant or replace command. The legacy replace library path
does not retain a durable pending journal and has no process-crash recovery
claim. Journal clear/unlink syscall fault or crash coverage and real-service
recovery at each crash boundary remain open.
...
| `caching_sha2_password` | n/a | n/a | experimental | experimental | partial | [`verifier`](server/src/verifier.rs), [`persistent account store`](server/src/persistent_account_store.rs), [`offline provisioning`](server/src/offline_provisioning.rs), [`offline CLI`](offline-provisioner/src/main.rs), [`runtime account store`](server/src/runtime_account_store.rs), [`checkpoint authority`](checkpoint-authority/src/lib.rs), [`reload supervisor`](server/src/runtime_account_reload_supervisor.rs), [`authentication state`](server/src/auth.rs), [`Unix protocol owner`](server/src/runtime_unix_connection.rs), [`Unix server`](server/src/runtime_unix_server.rs) | Constant-time verification uses one owned, zeroizing provider snapshot across fast/full auth and mints an opaque canonical account principal only on success. The persistent Unix store keeps only full verifiers and the matching privileges in one bounded, CAS-published generation; deleted account IDs remain retired, and open plus reload require exact equality with an external store-ID/revision/digest checkpoint. The runtime account-store waits on a one-shot checkpoint request with an explicit timeout before open and every serialized reload tick, then shares one `Arc` for credential lookup and authorization. Timeout cancels the request, keeps last-good command authorization for existing sessions, blocks new credential lookup and connection authorization, discards late results, and prevents another reload request until the old responder completes or disconnects. The local authority runs under a separate non-root UID, stores an exact durable high-water checkpoint, pins the client UID through kernel credentials, and bounds its protocol, workers, I/O, and state-lock waits. A privileged Linux Docker CI gate runs the real service and CLI as authorized and foreign numeric UIDs; it verifies `SO_PEERCRED` acceptance/rejection, owner/mode boundaries, and `SIGTERM` cleanup. The Unix-only CLI writes a durable pending initialization journal before snapshot publication and clears a snapshotless journal only after authority GET returns `Missing`; it accepts only one account with global `Connect`/`List`, not database grants or replacement. A deterministic process-kill test uses `SIGSTOP`, `SIGKILL`, then reconciliation at journal publication, snapshot publication, durable CAS, and journal removal. Test-only one-shot injection covers all sixteen before/after write, file-sync, rename, and directory-sync faults for journal/snapshot publication, with exact final-state, reconciliation, temporary-cleanup, and checkpoint-reopen checks. Initialize has a separate absolute password-input deadline; TTY cleanup uses `tcflush(TCIFLUSH)` and restores signal handlers, while FIFO/socket input temporarily uses `O_NONBLOCK` and restores flags. Full authentication is wired over the same-UID MySQL Unix transport and its deadline starts at admission. V1 is exact username-only. Journal clear/unlink syscall fault or crash coverage, real-service recovery at every crash boundary, journal-backed replacement, TCP/TLS, and certificate policy remain missing. |
...
server unit tests:      295 passed
checkpoint authority:  58 library + 3 binary tests passed

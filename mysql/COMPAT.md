...
operation, including a durable re-sync when retry sees an absent journal. The legacy
replace library path does not retain a durable pending journal and has no
process-crash recovery claim. A crash inside journal unlink, replacement
snapshot syscall faults, and real-service recovery at each crash boundary
remain open. Do not downgrade while a
replacement journal exists.
...
server unit tests:      323 passed
checkpoint authority:  60 library tests passed

...
runtime owner for the database catalog. SQL `USE` and database administration
remain closed. There is also no driver/ORM support promise yet.
...
| Classic packet framing and handshake | n/a | n/a | experimental | experimental | partial | [`mysql/server`](server/src/lib.rs), [`connection state`](server/src/connection_state.rs) | Bounded codecs and state transitions only; no production socket/TLS transport. Initial database selection must succeed before fast/full authentication emits its final OK; failure emits a typed ERR and closes. Accepted response-packet limits are at least 4096 bytes. |

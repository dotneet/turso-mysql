...
Four shapes are refused. An unnamed key, for the same reason the inline clause
...
it is implementing it that is left. `DROP KEY`, MySQL's other spelling for
`DROP INDEX`, because the parser library reads only the one. A statement mixing
index and column operations, which would have to apply two kinds of change
together. And the name `PRIMARY`, since adding or dropping a primary key is a
different operation than adding or dropping an index.

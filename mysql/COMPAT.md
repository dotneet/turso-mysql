...
Current working-tree evidence is recorded in the
[handoff](../docs/mysql-handoff-2026-09-04.md). Narrow ordering/limits,
empty-row default INSERT, `sql_notes`, `SHOW FULL TABLES`, and `DROP VIEW`
have focused test coverage and independent review approval after the `DROP`
...
rerun is verified. The earlier 50-test clean report remains a historical FAIL
artifact; the final clean profile also remains FAIL with seven mismatches and
no inconclusive reasons. The `drop_probe`, `create_probe`, `table_read`, and
...
collation was stripped. These gaps are not added to the committed feature
claims below. The final clean-profile report and
source provenance are retained at
`/tmp/turso-mysql-compare-rerun-20260905/results8/clean-profile.json` and
`/tmp/turso-mysql-compare-rerun-20260905/results8/source-provenance.txt`. A fresh isolated
...
from MySQL error 1101. `SHOW CREATE TABLE` remains unimplemented. The final
...
[operations guide](../docs/mysql-checkpoint-authority.md). The final recorded
...
| Narrow `information_schema.COLUMNS` contract | partial | n/a | experimental | n/a | experimental | [`checked parser`](parser/lib.rs), [`frontend adapter`](server/src/frontend_adapter.rs), [oracle case](conformance/cases/p0/information-schema-columns.json), [P0 manifest](conformance/Makefile) | The exact `COLUMN_NAME`/`ORDINAL_POSITION`/`COLUMN_DEFAULT`/`IS_NULLABLE`/`COLUMN_TYPE`/`COLUMN_KEY`/`EXTRA` projection, `DATABASE()`/`records` filter, and ordinal ordering are accepted by the checked parser and fixed provider. The narrow unnamed explicit column-`NULL` form is durable and its frontend metadata is tested, including the restored `MEDIUMINT NULL` fixture. Named or conflicting nullable attributes remain rejected. Selected-database `Query` authorization runs before lookup, with the narrow table `Select` fallback; missing or denied `records` returns an empty result. Golden metadata is pinned, and scan, row, value, packet-payload, and retained-memory bounds are checked before staging output. Other providers and cross-database coverage remain incomplete. |

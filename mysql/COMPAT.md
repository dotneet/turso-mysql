...
No feature is currently classified as `supported`. TCP/TLS listening and TLS
termination are not implemented. A standalone Unix-only
...
| Prepared commands | partial | partial | n/a | partial | partial | [`mysql/frontend`](frontend/session.rs), [`frontend adapter`](server/src/frontend_adapter.rs), [D012 core contract](../core/dialect/mod.rs) | `COM_STMT_PREPARE`/`EXECUTE`/`RESET`/`CLOSE` support checked `SELECT` and conservative ordinary `INSERT`/`UPDATE`/`DELETE`; SELECT results use binary rows and writes return OK effects. Binary parameter decoding, cached parameter types, and schema reprepare snapshots are covered. `COM_STMT_SEND_LONG_DATA` appends binary or text chunks without a response, retains at most 8 MiB per connection, defers errors until execute, and clears staged data on execute, successful reset, or close. The AUTO_INCREMENT case is limited to the documented omitted-ID bare-`?` `VALUES` form. Cursor modes, exact long-data error diagnostics, prepared DDL, prepared transaction commands, and wider SQL remain rejected. |
...
| Driver and ORM compatibility | planned | n/a | experimental | planned | experimental | [D010/P6 plan](../docs/mysql-compatibility-plan.md), [`mysql_async` Unix E2E](runtime/tests/unix_e2e.rs) | The only external-driver pilot is `mysql_async = "=0.37.1"` over a Unix socket, using default `OptsBuilder` values (no explicit `max_allowed_packet` or `wait_timeout`). Its exact bootstrap query is `SELECT @@max_allowed_packet,@@wait_timeout`. The ignored privileged Linux E2E covers authentication, `USE`, text DDL/DML, prepared DML, reads, independent connection state, reconnect, and `SIGTERM` cleanup. TCP/TLS, pool reset semantics, and ORMs are outside this pilot; no general driver or ORM version is promised. |
...
```text
frontend unit tests:    195 passed
parser unit tests:      49 passed
server unit tests:      407 passed
runtime tests:            5 passed, 1 ignored (privileged Linux cross-UID CI fixture)

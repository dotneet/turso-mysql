...
socket listener, TLS termination, provisioning executable, external
rollback-checkpoint service, or runtime owner for the database catalog. A
persistent Unix account/privilege backend, default-deny authorization port, and
post-authentication wrapper are present. Offline provisioning, pure runtime
security configuration, and an externally checkpointed runtime account-store
boundary exist as library components. Trusted embedded sessions and
...
| `caching_sha2_password` | n/a | n/a | experimental | experimental | partial | [`verifier`](server/src/verifier.rs), [`persistent account store`](server/src/persistent_account_store.rs), [`offline provisioning`](server/src/offline_provisioning.rs), [`runtime account store`](server/src/runtime_account_store.rs), [`authentication state`](server/src/auth.rs) | Constant-time verification uses one owned, zeroizing provider snapshot across fast/full auth and mints an opaque canonical account principal only on success. The persistent Unix store keeps only full verifiers and the matching privileges in one bounded, CAS-published generation; deleted account IDs remain retired, and open plus reload require exact equality with an external store-ID/revision/digest checkpoint. The runtime account-store reads that checkpoint before open and every explicit, serialized reload tick, then shares one `Arc` for credential lookup and authorization. A failed tick keeps last-good command authorization for existing sessions but blocks new credential lookups and connection authorization until a later exact reload succeeds. Offline provisioning clears borrowed password buffers and acknowledges only after external checkpoint CAS. V1 is exact username-only. No provisioning executable, external checkpoint service, listener, scheduler, bounded checkpoint reader, TLS engine, certificate policy, or bind-time final checkpoint recheck exists. |
...
| TCP/TLS and Unix-socket listeners | n/a | n/a | planned | planned | planned | [`runtime config`](server/src/runtime_config.rs), [`runtime account store`](server/src/runtime_account_store.rs), [protocol architecture](../docs/mysql-compatibility-mode.md) | Pure configuration requires TCP TLS references, same-effective-UID Unix peer policy, an external checkpoint authority, and bounded limits/timeouts. The runtime account-store consumes an injected checkpoint reader for initial open and explicit serialized reload ticks, but it owns neither listener nor scheduler. No listener, TLS engine, certificate policy, peer inspection, serial worker runtime, or final exact checkpoint recheck immediately before bind exists. |

...
| `SET` | not started; it is `ENUM`'s shape with several members at once, and what it stores is a different question |
| `JSON` | not started. The seam that blocked both is open: a **quoted** declared type is taken by the engine whole and given back unchanged, so a MySQL type the engine cannot otherwise spell rides on the declared type name like every other. A table's durable row already **holds MySQL DDL** — the envelope is `/*@turso:mysql-schema:v2:...*/ <MySQL DDL>` and the schema loader parses that back — but the text stored at creation is rendered out of the **engine's** AST rather than the MySQL one: `prepare_schema_ddl` calls `render_create_table_mysql_with_mode(&stmt, mode)` on the translated statement, so a type the engine cannot spell is already gone before it is written. The engine's declared type name is what carries `INT UNSIGNED` and `VARBINARY(255)` through today. The fix is to render the durable DDL from the MySQL AST the parser already has, and the shape of it exists: `table_sql_for_replay` in `frontend/dialect.rs` keeps `decoded.normalized_ddl` verbatim for the `AUTO_INCREMENT` and checked-primary-key tables for exactly this reason. The engine's declared type takes a word before its arguments and numbers inside them, and `ENUM('a','b')` is neither: measured, `CREATE TABLE e (s ENUM('a','b'))` answers `near "'small'": syntax error` in the engine. Keeping the members would mean keeping the original MySQL DDL durably, the way the v2 `AUTO_INCREMENT` metadata already does for its own facts. There is a second thing to decide after that: measured on 8.4.11, `ORDER BY` on an `ENUM` orders by the member's declared position, so `small, medium, large` come back in that order and not alphabetically |

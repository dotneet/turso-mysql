...
| `DATE(col)`, `TIME(col)` | refused; `CAST(col AS DATE)` reads the day out and is taken, but the one-word spelling is not classified yet |
...
| A `WHERE` comparison against a `BLOB` column | refused; **measured on 8.4.11**, so the work is implementing it: MySQL compares the bytes, `payload = 'ABC'` finding no row holding `abc` and `payload > 'a'` reading them in byte order. That is the engine's own comparison, so what it needs is the renderer being told a column is binary and leaving off the collation a text column asks for — the same channel that tells it a column is text |

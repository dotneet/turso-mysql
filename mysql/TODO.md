...
| `DATE_ADD` / `DATE_SUB` over a reading of the moment — `DATE_SUB(NOW(), INTERVAL 30 DAY)` | refused; both take a column as the thing to shift, and the result's shape is worked out from that column's type. **Measured on 8.4.11**, so the work is implementing it: over `NOW()` any interval answers a nullable `DATETIME` of length 19 in the binary character set; over `CURDATE()` an interval of whole days, months or years answers a nullable `DATE` of length 10 and a time interval answers a nullable `DATETIME` of length 19. `NOW()` and `CURDATE()` themselves are not null; every shift of one is nullable. `WHERE dt > DATE_SUB(NOW(), INTERVAL 30 DAY)` is the shape a suite writes, so the comparison reader needs it too |

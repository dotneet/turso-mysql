...
| `ALTER TABLE ... ADD COLUMN` on a counted table carrying a foreign key | leaves the table unreadable; measured against this server, `SHOW CREATE TABLE`, `SHOW COLUMNS` and `information_schema.COLUMNS` all fail afterwards where they answered the table before. Traced: two renderers spell a foreign key differently. The counted table's own renderer writes what `sqlparser` prints — `CONSTRAINT b2_fk FOREIGN KEY (owner) REFERENCES b1(id)` — and the engine's schema rewrite writes `FOREIGN KEY (\`owner\`) REFERENCES \`b1\` (\`id\`)`, dropping the constraint name and quoting the rest. The read path holds the stored DDL to being exactly what the counted renderer would write, so after the rewrite it is not, and every reader answers `CorruptDefinition`. Making the two agree changes what is stored, so a table written by the older spelling would stop reading unless the check takes both |

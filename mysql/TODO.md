...
| `ALTER TABLE ... ADD COLUMN` on a counted table carrying a foreign key | leaves the table unreadable; measured against this server, `SHOW CREATE TABLE` answers 1235 afterwards where it printed the table before. The positioned form goes through the rewrite and is unaffected; the plain one goes through the engine's own `ALTER`, and what that leaves in the stored DDL has not been traced |
